...
static void look_north(zappy_t *zappy, int ci)
{
    ...
    for (int i = 1; i <= player->level; ++i) {
        ...
        for (int j = player->x - i; j <= player->x + i; ++j) {
            ...
            sdprintf(zappy, client_socket(ci), "%s%s", res, (i == player->level && j == player->x + i) ? "" : ",");
            free(res);
        }
    }
...
static void look_east(zappy_t *zappy, int ci)
{
    ...
    for (int i = 1; i <= player->level; ++i) {
        ...
        for (int j = player->y - i; j <= player->y + i; ++j) {
            ...
            sdprintf(zappy, client_socket(ci), "%s%s", res, (i == player->level && j == player->y + i) ? "" : ",");
            free(res);
        }
    }
    sdprintf(zappy, client_socket(ci), "]\n");
...
{
    (void)command;
    zappy->game.actions = add_action(zappy->game.actions, time_limit(7), strdup(command), ci, look);
}
